...
- The `disableThinking` provider option is the config-file equivalent of `CLAUDE_CODE_DISABLE_THINKING=1`. It is OR'd into `isClaudeThinkingDisabled(configDisableThinking)` / `claudeSpawnEnv(configDisableThinking)` in `session-manager.ts`; the env vars still force thinking off regardless (they remain highest priority). Threaded via `self.config.disableThinking` into both spawn paths (`spawnClaudeProcess` and the raw `doGenerate` spawn) plus `thinkingCliOptions()`. Reason it exists: some Claude Code CLI versions corrupt the extended-thinking signature round-trip across turns in the reused stream-json process, so Anthropic rejects the next request with `400 ... thinking or redacted_thinking blocks in the latest assistant message cannot be modified`. That is an upstream CLI bug; this plugin builds requests correctly but is the layer that opts users into thinking by default, so the escape hatch lives here. Tests in `test-cli-args.ts`.
