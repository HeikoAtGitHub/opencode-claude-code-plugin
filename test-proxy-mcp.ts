...
test("governance proxy defs retain narrow schemas and the pinned workstream contract", () => {
  const byName = new Map(DEFAULT_PROXY_TOOLS.map((tool) => [tool.name, tool]))
  assert.ok(byName.has("submit_plan"))
  assert.equal(resolveProxyCallTimeoutMs("submit_plan", undefined, undefined), 24 * 60 * 60 * 1000)

  const repo = byName.get("repo_policy_scope")!
  assert.deepEqual(repo.inputSchema.required, ["targets"])
  assert.equal((repo.inputSchema as any).additionalProperties, false)

  const workstream = byName.get("workstream_manage")!
  assert.equal((workstream.inputSchema as any).type, "object")
  assert.deepEqual((workstream.inputSchema as any).required, ["action"])
  assert.deepEqual((workstream.inputSchema as any).properties.action.enum, WORKSTREAM_ACTIONS)
  assert.ok((workstream.inputSchema as any).properties.authorized_through)
  assert.equal(WORKSTREAM_TRANSPORT_CONTRACT_VERSION, "1.6.0")
})

test("request-scoped governance tools require request and live native availability", () => {
  const selected = DEFAULT_PROXY_TOOLS.filter((tool) =>
    ["bash", "repo_policy_scope", "workstream_manage"].includes(tool.name),
  )
  const onlyRepo = filterRequestScopedProxiesByAvailability(
    selected,
    new Set(["bash", "repo_policy_scope", "workstream_manage"]),
    new Set(["bash", "repo_policy_scope"]),
  )
  assert.deepEqual(onlyRepo.map((tool) => tool.name), ["bash", "repo_policy_scope"])
  assert.notEqual(proxyToolExposureHash(selected), proxyToolExposureHash(onlyRepo))
})

test("governance proxy input validation rejects widened transport payloads", () => {
  assert.equal(validateProxyToolInput("repo_policy_scope", { targets: ["/tmp/repo"] }), null)
  assert.match(
    validateProxyToolInput("repo_policy_scope", { targets: ["/tmp/repo"], force: true })!,
    /accepts only targets/,
  )
  assert.equal(validateProxyToolInput("workstream_manage", { action: "list" }), null)
  assert.match(
    validateProxyToolInput("workstream_manage", { action: "sync_push", slug: "demo", remote: "origin" })!,
    /accepts only/,
  )
  assert.match(
    validateProxyToolInput("workstream_manage", { action: "collision_preview", slug: "demo", planned_paths: ["../escape"] })!,
...
test("resolveProxyClientCeilingMs covers the largest effective deadline", () => {
  ...
  // the largest effective deadline. The local submit_plan bridge deliberately
  // keeps a 24-hour human-review window, so it remains the maximum here.
  assert.equal(
