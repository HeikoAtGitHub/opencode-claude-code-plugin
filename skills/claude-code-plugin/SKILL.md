...
| `proxyTools` | string[] | `["Bash", "Edit", "Write", "WebFetch", "Task"]` | Case-insensitive replacement list, not additive and not a capability allowlist. Known entries expose `mcp__opencode_proxy__<name>`; omitted/unknown tools are not disabled. `Task` also brings `task_batch`. Local-variant entries: `submit_plan`, `repo_policy_scope`, `workstream_manage`; the last two are request-scoped and fail closed without exact session/caller context. `[]` disables this list, not MCP proxying. |
